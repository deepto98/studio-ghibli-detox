...
export default function ImageUploadArea() {
  ...
  const [processingMessage, setProcessingMessage] = useState("Analyzing Ghibli contamination...");
  ...
  const analyzeImage = useMutation({
    ...
    onError: (error: any) => {
      ...
      if (error.response && typeof error.responseBody === 'object' && error.responseBody.message) {
      ...
      if (errorMessage.includes("invalid_image_format") || 
          errorMessage.includes("Image format error") ||
          errorMessage.includes("Invalid input image")) {
        errorMessage = "Image format error: Please upload a valid JPG, PNG, or WEBP file. The image will be automatically converted to PNG with transparency for processing.";
      } else if (errorMessage.includes("file too large") || 
                errorMessage.includes("exceeds the size limit")) {
        errorMessage = "Image is too large. Please upload an image smaller than 4MB.";
      }
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      ...
      try {
        ...
        toast({
          title: "Link Copied!",
          description: "Shareable link copied to clipboard. You can now paste and share it.",
        });
      } catch (error) {
        toast({
          title: "Copy Failed",
        ...
        {uploadState === "initial" && (
          ...
          >
            <div className="space-y-4">
              ...
              <h3 className="text-lg font-medium text-gray-800">Upload an image for detoxification</h3>
              <p className="text-sm text-gray-600">
                Drag and drop your Ghibli-contaminated image here, or click to select
              </p>
              <div>
              ...
              <p className="text-xs text-gray-500">Supports JPG, PNG, and WEBP up to 10MB</p>
            </div>
          </div>
        )}
            ...
            <div className="space-y-4">
              <div className="relative w-20 h-20 mx-auto">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-sm font-medium text-blue-800">{progressPercent}%</span>
                </div>
              </div>
              
              <h3 className="text-lg font-medium text-blue-800">{processingMessage}</h3>
              <p className="text-sm text-blue-600">
                Our AI doctors are examining your image for signs of excessive whimsy
              </p>
            </div>
          </div>
        )}
        ...
        {uploadState === "results" && analysis && originalImage && (
          <div>
            ...
            <div className="mt-8 p-4 bg-gray-50 rounded-lg border border-gray-100">
              <h3 className="text-sm font-medium mb-3 text-gray-700">Ghibli Contamination Level</h3>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              >
                <Download className="h-4 w-4 inline mr-1" /> Download Detoxified Image
              </button>
              <button 
